import { defaultCerebrasModel } from "./cerebrasModelConfig";

const CEREBRAS_CHAT_COMPLETIONS_URL =
  "https://api.cerebras.ai/v1/chat/completions";
const CEREBRAS_TIMEOUT_MS = 30_000;
const CEREBRAS_RETRIES = 2;
...
async function cerebrasChat(
  ...
  const model = options.model ?? defaultCerebrasModel();
  let lastError: unknown = null;

  for (let attempt = 0; attempt <= CEREBRAS_RETRIES; attempt += 1) {
    const startedAt = Date.now();
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), CEREBRAS_TIMEOUT_MS);

    try {
      console.log("[dotti:cerebras] request", {
        stage: options.stage,
        model,
        attempt: attempt + 1,
      });
      ...
      if (!response.ok) {
        ...
        console.error("[dotti:cerebras] failure", {
          stage: options.stage,
          model,
          attempt: attempt + 1,
          durationMs,
        ...
        const error = new Error(
          `Cerebras ${options.stage} failed with HTTP ${response.status}: ${message}`,
        );
        lastError = error;

        if (!isTransientStatus(response.status) || attempt >= CEREBRAS_RETRIES) {
          throw error;
        }
      ...
      const content = cleanText(
        contentToText((json as CerebrasChatResponse | null)?.choices?.[0]?.message?.content),
      );
      console.log("[dotti:cerebras] success", {
        stage: options.stage,
        model,
        attempt: attempt + 1,
        durationMs,
        promptTokens: (json as CerebrasChatResponse | null)?.usage?.prompt_tokens,
        completionTokens:
          (json as CerebrasChatResponse | null)?.usage?.completion_tokens,
        totalTokens: (json as CerebrasChatResponse | null)?.usage?.total_tokens,
      });
      ...
      if (isAbortError(error)) {
        console.error("[dotti:cerebras] timeout", {
          stage: options.stage,
          model,
          attempt: attempt + 1,
          durationMs,
      ...
      } else if (!(error instanceof Error && error.message.startsWith("Cerebras "))) {
        console.error("[dotti:cerebras] failure", {
          stage: options.stage,
          model,
          attempt: attempt + 1,
      ...
      if (attempt >= CEREBRAS_RETRIES) break;
      if (error instanceof Error && error.message.startsWith("Cerebras ")) break;
  ...
  if (isAbortError(lastError)) {
    throw new Error(
      `Cerebras ${options.stage} timed out after ${CEREBRAS_TIMEOUT_MS / 1000}s`,
    );
  }
...
export async function cerebrasChatJson(
  systemPrompt: string,
  userPrompt: string,
  options: CerebrasChatOptions,
): Promise<string> {
  return cerebrasChat(systemPrompt, userPrompt, {
    ...options,
